...
async function processJob(job) {
  ...
  return context.with(parentContext, async () => {
    ...
    try {
      ...
      const restoredBuffer = Buffer.from(restorationResult.restoredImage, 'base64');
      const stored = await clients.gcs.saveRestoredImage({
        userId: job.data.userId,
        jobId: job.data.jobId,
      ...
      await clients.firestore.collection('restorations').doc(job.data.jobId).set({
        ...
        resultObjectName: stored.objectName,
        updatedAt: new Date(),
      }, { merge: true });
